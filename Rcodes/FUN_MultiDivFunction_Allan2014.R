...
## x is a dataframe with different plots on different rows and different groups in different columns

multidiv <- function(x, threshold=FALSE, sc = "max", cent = FALSE, by =FALSE, weights = FALSE){
  
  result <- matrix(nrow=nrow(x),ncol=2)
  
  if(any(by!=FALSE)){  ## Scaling
  ...
  if(FALSE %in% threshold){   ### prevent error message if threshold is vector
    m <- apply(x.stand, 1, mean, na.rm=T)  # when threshold is false, m is the average of scaled values
  }
